...
class SpatialHistoryPredictor(nn.Module):
    def __init__(self, latent_grid, history_steps, hidden=128, kind='mlp', substeps=2,
        ...
        self.kind, self.substeps, self.information_dim = kind, substeps, 0
        self.periodic_lon = bool(periodic_lon)
        channels = self.latent_grid[0]
        # Full observed history becomes a spatial context field. No future field
        # or auxiliary-information bypass enters this predictor.
        self.context = nn.Sequential(
            GeographicConv2d(history_steps * channels + 4, hidden, periodic_lon=periodic_lon), nn.SiLU(),
    ...
    def forward(self, history, lead_hours, origin_ns, information=None):
        ...
        context = self.context(torch.cat((observed.flatten(1, 2), calendar), dim=1))
