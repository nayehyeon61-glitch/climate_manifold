...
class ForecastPipeline(nn.Module):
    def __init__(self, manifold, config, constants=None, schema=None):
        super().__init__()
        self.config, self.a_config = config, manifold.config
        self.bridge = ManifoldBridge(manifold, config.bridge, config.anchor)
    ...
    def forward(self, history, information, origin_ns, lead_hours):
        ...
        if self.predictor is None:
            ...
            predicted,std = self.predictor(features,lead_hours,origin_ns,information)
